...
const BookingHistory = () => {
  ...
  const { booking, travelers, status, bookingId, pricing } = state;
      ...
      <section className="rounded-3xl bg-white shadow-2xl border border-orange-100 overflow-hidden p-6 sm:p-8">
        ...
        <div className="bg-gradient-to-r from-orange-50 to-orange-100 p-4 sm:p-6 rounded-2xl mb-6">
          ...
          <div className="mt-4 flex items-center justify-between text-slate-700">
            <p><span className="font-medium">Package:</span> {booking.package}</p>
            <p><span className="font-medium">Email:</span> {booking.email}</p>
          </div>
        </div>

        {/* ---------- Passenger Details + Invoice Section ---------- */}
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-1">
              {travelers.map((traveler, index) => (
                ...
                >
                  ...
                  <p className="text-sm text-slate-700">{traveler.age} yrs • {traveler.gender}</p>
          ...
          >
            <div className="grid grid-cols-2 gap-4 text-black">
              ...
              <p><span className="font-medium">Package:</span> {booking.package}</p>
              <p><span className="font-medium">Number of Travelers:</span> {travelers.length}</p>
              <p><span className="font-medium">Price per Person:</span> ₹{pricing.perPerson.toLocaleString()}</p>
              <p><span className="font-medium">Total Price:</span> ₹{pricing.total.toLocaleString()}</p>
            </div>
            <button
